...
def decode_rows(path: Path, manifest: dict[str, Any]) -> Iterator[dict[str, Any]]:
    ...
    with path.open("rb") as handle:
        for record_index in range(int(manifest["record_count"])):
            ...
            flags = int(values.pop("flags"))
            packed = int(values.pop("packed_new_separators"))
            yield {
                "record_index": record_index,
                **values,
                "flags": flags,
                "internal_edge_count": flags & 0b11,
                "decoded_flags": decode_flags(flags, manifest["flag_bits"]),
                "packed_new_separators": packed,
                "new_vertex_separator_mask": packed & 0b111,
                "new_edge_separator_count": packed >> 3,
            }
        if handle.read(1):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Decode the Fase-104 triple-synergy ledger")
