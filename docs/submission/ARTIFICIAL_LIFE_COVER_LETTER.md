...
The main scientific case study concerns a periodic-background oscillator family in `rule_73/rule_109`. A length-8 background sweep identifies a `T=15` five-state locking mechanism, and a 25-cell, 12-step causal cone reveals an algebraic normal form (ANF) gradient with `R^2 = 0.998197`. A global detector audit then replays 5,783,040 configurations and confirms 3,296 stationary oscillators omitted by the historical `T<=16` detector. Quotienting that population yields 192 strict attractor classes and 48 complete Q8 intervention cubes. On 43,425 unit interventions to 219 fragile targets, restored vertex or edge redundancy is exactly equivalent to bypassing every corresponding critical cut; direct cut removal and independent max-flow agree without exception. Exhaustive pair, triple, and quadruple audits then resolve the collective minimum-cardinality ladder completely at values two through four within the frozen protocol. An exact motif audit classifies all 27,828,370 candidates and compiles the 1,476 verified metric-specific rescues into 265 sparse unit-cost QUBOs with exactly 1,476 certified ground states; no quantum hardware is used and no quantum advantage is claimed. A conditioned follow-up then shows, for 122 rescues in 24 mixed `K2` instances, that a fixed budget of 14 external Hamming-1 incidences is allocated differently between the period-specific candidate universe and the disjoint bridge graph. This is a descriptive result of the frozen census, not an externally validated classifier. The resulting claims remain empirical and protocol-bounded: ZUSE is presented as a reproducible evidence engine for CA law discovery, not as a universal autonomous scientist.
